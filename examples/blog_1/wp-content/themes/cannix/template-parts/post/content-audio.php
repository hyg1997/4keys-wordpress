...
<article id="post-<?php the_ID(); ?>" <?php post_class( $post_class ); ?><?php echo cannix_inline_article_style( true, true ); ?>>
	...
	<?php else: ?>
		...
		if ( is_single( ) ) {
			...
			// Feature the audio file.
			if ( ! empty( $audio ) ) {
						echo '<div class="video-wrapper">';
							echo cannix_audio_embed();
						echo '</div>';
			};

		} else {

			if ( '' === get_the_post_thumbnail() && empty( $audio ) ) {

				if ( $placeholder ) {

					// Not a single post no thumbnail and no audio so display a placeholder
